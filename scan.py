...
def packet_to_row(pkt):
    if IP in pkt:
        ...
        return [time.time(), pkt[IP].src, pkt[IP].dst, pkt.proto, sport, dport, flags, len(pkt), 'scan']
    return None
...
with open(output_file, 'w', newline='') as f:
    ...
    sniff(filter="tcp", prn=process, store=0, count=100)
